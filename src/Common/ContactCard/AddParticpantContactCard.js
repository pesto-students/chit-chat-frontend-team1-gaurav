...
function AddParticpantContactCard({chatDetails,selectedContacts,setSelectedContacts}) {
  ...
  let clickHandler = () => {
    let found = selectedContacts.find((item) => {
      return item.userid === chatDetails.userid;
    });
    ...
    if (found) {
      ...
      setSelectedContacts((prev) => [
        ...prev,
        { userid: chatDetails.userid, username: chatDetails.username },
      ]);
    }
  };
...
export default AddParticpantContactCard;
